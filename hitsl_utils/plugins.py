...
#
# TODO: Пусть юристы скажут, как правильно оформить Copyright Notice


import blinker


def signal_send(name, sender, *args, **kwargs):
    blinker.signal('hitsl.utils.plugins.%s' % name).send(sender, *args, **kwargs)


def signal_subscribe(name, *args, **kwargs):
    blinker.signal('hitsl.utils.plugins.%s' % name).connect(*args, **kwargs)


__all__ = ['Component', 'ExtensionPoint', 'implementer', 'Interface', 'JustError']
...
class JustError(Exception):
    ...
    def __init__(self, message, title=None, show_traceback=False):
        ...
        message.
        """
        super(JustError, self).__init__(message)
...
class ComponentMeta(type):
    ...
    def __new__(mcs, name, bases, d):
        ...
        is_abstract = d.pop('abstract', False)
        new_class = type.__new__(mcs, name, bases, d)
        new_class.was_abstract = is_abstract
        ComponentMeta.__register(new_class)
        return new_class
    ...
    def __call__(cls, *args, **kwargs):
        ...
        if self is None:
            ...
            signal_send('component_activated', compmgr, component=self)
            self.__init__()
    ...
    def implementer(*interfaces):
        def decorator(klass):
            _implements = getattr(klass, '_implements', [])
            _implements.extend(interfaces)
            setattr(klass, '_implements', _implements)
            ComponentMeta.__register(klass)
            return klass
        return decorator

    @staticmethod
    def __register(new_class):
        abstract = getattr(new_class, 'was_abstract', False)
        if abstract:
            # Don't put abstract component classes in the registry
            return new_class

        if new_class not in ComponentMeta._components:
...
implementer = ComponentMeta.implementer
    ...
    def __getitem__(self, cls):
        ...
        if not component and not issubclass(cls, ComponentManager):
            if cls not in ComponentMeta._components:
                raise JustError('Component "%s" not registered' % cls.__name__)
            try:
                component = cls(self)
            except TypeError, e:
                raise JustError("Unable to instantiate component %r (%s)" % (cls, e))
        return component
...
class PluginExtension(Component, ComponentManager):
    def __init__(self, app=None):
        super(PluginExtension, self).__init__()
        self.app = app
        if app:
            self.init_app(app)
